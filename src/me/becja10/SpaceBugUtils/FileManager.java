...
import me.becja10.SpaceBugUtils.SpaceBugUtils;
...
public class FileManager
{
	...
	public static void reloadPlayers() {
		if (players == null)
			players = new File(SpaceBugUtils.getInstance().getDataFolder(), "players.yml");
		config = YamlConfiguration.loadConfiguration(players);

		InputStream defConfigStream = SpaceBugUtils.getInstance().getResource("players.yml");
	...
	public static void savePlayers() {
		...
		try {
			...
			System.out.println("[SpaceBugUtils] Could not save config to " + players);
		}
	}
	...
	public static void saveDefaultPlayers() {
		if (players == null)
			players = new File(SpaceBugUtils.getInstance().getDataFolder(), "players.yml");
		if (!players.exists())
			SpaceBugUtils.getInstance().saveResource("players.yml", false);
	}
}